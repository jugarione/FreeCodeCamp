...
d3.select('body') //selects an element in the document
.selectAll("h2") // sellects all the h2 elements
.data(dataset) // get the data elements
.enter() // create elements to represent the data
.appends("h2") // appends an element and return a handle to the html
.text("jamon") // insert the data in the elements
.text((d) => d + " USD");  // takes the elements of the array and display it, returns "item USD"
.style("font-family","verdana") // let yo add css
.style("color", (d) => { // you can style based on the data
        if(d <= 20){ return "red"}
        return "green"
      })
.attr("class", "bar") // lets you add a Styling class in css

.append("svg") // stands for Scalable Vector Graphics. 11
.append("rect") //makes a renctangle in the svg
                  .attr("x","0")
                  .attr("y","0")
                  .attr("height","100")
                  .attr("width","25")
// separates all rect elements using the index by 30px
.attr("x", (d, i) => {
         i = dataset.indexOf(d) * 30;
         return i;
       })
// upside-down, ORIGIN 0,0 IS UPPER-LEFT coRNER
.attr("y", (d, i) => {
        i = 100 - d * 3;
        return i
       })

// svg text value, needs x and y and the d3 text value
svg.selectAll("text")
       .data(dataset)
       .enter()
       .append("text")
...
### actualize before the page chargues
...
### changue text with click events
